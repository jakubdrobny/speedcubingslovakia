...
func ExtractPayload(tokenString *jwt.Token) (AuthDetails, error) {
	...
	if !ok {
		return AuthDetails{}, fmt.Errorf("extracting payload from token failed")
	}

	var authDetails AuthDetails
	uidFloat, ok := claims["userid"].(float64)
	if !ok {
		return AuthDetails{}, fmt.Errorf("failed to parse userid")
	}
...
func VerifyJWTToken(tokenString string, secretKey string) (AuthDetails, error) {
	...
	if !token.Valid {
		return AuthDetails{}, fmt.Errorf("invalid token")
	}
...
func GetAuthDetailsFromHeader(c *gin.Context, secretKey string) (AuthDetails, error) {
	...
	if len(headers) <= 0 {
		...
		if err != nil {
			token = c.DefaultQuery("atoken", "")
			if token == "" {
				return AuthDetails{}, fmt.Errorf("could not get cookie from gin.Context")
			}
		}
	...
	if len(headers) <= 0 {
		return AuthDetails{}, fmt.Errorf("auth header missing")
	}

	header := strings.Split(headers[0], " ")
	if len(header) < 2 || header[0] != "Bearer" {
		return AuthDetails{}, fmt.Errorf("bad auth header")
	}
