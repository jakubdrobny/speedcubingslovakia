import { Alert, Box, Button, Card, Chip, CircularProgress, FormControl, FormHelperText, FormLabel, Input, Option, Select, Stack, Typography } from "@mui/joy";
import { AuthContextType, CompetitionData, CompetitionEvent, CompetitionState } from "../../Types";
import { formatCompetitionDateForInput, getAvailableEvents, initialCompetitionState, updateCompetition } from "../../utils";
...
const CompetitionEdit: React.FC<CompetitionEditProps> = ({ edit }) => {
    ...
    const handleSelectedEventsChange = (selectedEventsNames: string[]) => {
        const selectedEvents = selectedEventsNames.map(eName => availableEvents.find(e => e.displayname === eName) as CompetitionEvent);
        setCompetitionState({...competitionState, events: selectedEvents});
    };

    const handleSubmit = () => {
        if (!competitionState.name || !competitionState.startdate || !competitionState.enddate || !competitionState.events)
            return;
    ...
    return (
        <div style={{marginTop: "2em"}}>
            ...
            <Card>
                <Typography level="h3" sx={{borderBottom: "1px solid #CDD7E1"}}>{edit ? `Edit ${competitionState.name}` : 'Create'} competition</Typography>
                <Stack spacing={2}>
                    <FormControl>
                        <FormLabel>
                    ...
                    <FormControl>
                        <Button onClick={handleSubmit}>{edit ? "Edit" : "Create"} competition</Button>
                    </FormControl>
                </Stack>
            </Card>}
        </div>
